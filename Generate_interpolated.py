...
def make_data(df, adm_grp):
    ...
    dates.to_csv(os.path.join(output_dir, '{}_flood_peak.csv'.format(adm_grp)), index=False)
    flood_extents.to_csv(os.path.join(output_dir, '{}_flood_extent_interpolated.csv'.format(adm_grp)), index=False)
...
if __name__ == "__main__":
    ...
    try:
        sentinel = pd.read_csv(os.path.join(output_dir, '{}_flood_extent_sentinel.csv'.format(arg.adm_level)))
